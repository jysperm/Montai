...
- **Less structured LLM outputs**: Prompts prefer free-form prose or Markdown over rigid JSON schemas for intermediate text (e.g. project summaries, storyline narratives). Only outputs that are consumed programmatically (e.g. VideoSummary, EditSpec) use structured JSON. This gives the LLM more flexibility and produces more natural text. Use examples in prompts to guide the expected content rather than prescribing exact schemas.
...
`intermediateLanguage` controls the language used by the LLM for all generated text: video analysis descriptions, project summaries, storyline narratives, and edit spec text content (titles, captions). Supports `zh` (Chinese) or `en` (English), defaults to `en`. This is separate from `effects.languages`, which controls subtitle/caption language variants in the final output.
...
- **project_summary** — Incrementally updated cross-video summary (free-form text)
- **storylines** — Generated narrative structures (JSON)
- **edit_specs** — Concrete editing instructions (JSON), each with a unique `name` per project
...
For each unanalyzed video, in sequence:

1. **Preprocess**: Transcode video via ffmpeg to reduce upload size (1 FPS, 720p 8-bit, mono audio 64kbps). Transcoded files are cached in `.cutflow/transcoded/` (in the project directory) and reused until the source file changes
2. Upload video to LLM File API (or reuse cached ref if still active)
3. Send video + prompt to get structured VideoSummary JSON
4. Store VideoSummary in `video_summaries` table
5. In the same LLM context (multi-turn), send the current ProjectSummary and ask the model to produce an updated ProjectSummary incorporating this video's content
6. Store updated ProjectSummary in `project_summary` table

Supports resume: skips videos that already have a row in `video_summaries` on re-run.

### 2. Storyline (`cutflow storyline`)

Text-only LLM call. Sends project summary + all video summaries, receives structured Storyline JSON defining the narrative arc and clip selection.

### 3. Edit (`cutflow edit`)

Runs as an **agent loop** where the model iteratively builds the EditSpec:

1. Start with storyline + video summaries
2. Draft an initial EditSpec
3. Model can request to re-watch specific video segments (via LLM File API with `videoMetadata` start/end offsets)
4. Model updates EditSpec based on what it sees
5. Loop terminates when model returns final EditSpec or max iterations reached
...
Outputs EditSpec to `.cutflow/specs/<name>.json` and FCPXML file.

### 4. Render (`cutflow render [name]`)

Loads the EditSpec from the database (by name, or latest if omitted), prepares a public directory with hard links to video files, then runs `npx remotion render` against CutFlow's built-in static Remotion project with `--props` and `--public-dir` flags. Output goes to `output/<name>.mp4`.

### 5. Studio (`cutflow studio [name]`)

Loads the EditSpec from the database (by name, or latest if omitted), prepares a public directory (including `editSpec.json` for studio fallback), then runs `npx remotion studio` against CutFlow's built-in static Remotion project with `--public-dir`.

## Edit Spec Data Model

The EditSpec is the shared intermediate representation consumed by both Remotion and FCPXML generators.

A project can produce multiple EditSpecs (multiple output videos). Each EditSpec has a unique `name` used as an identifier and output filename. Each render/studio invocation operates on a single EditSpec, passed to the static Remotion project via `--props`.
...
- **Single Composition** (`CutFlow`): Uses `calculateMetadata` to compute duration/dimensions from the EditSpec
- **Render mode**: EditSpec passed via `--props=<path>`, video files served via `--public-dir=<path>`
- **Studio mode**: EditSpec fetched from `staticFile('editSpec.json')` when props are empty, video files served via `--public-dir=<path>`
- **Public dir**: `render`/`studio` commands create `.cutflow/public/` with hard links to source video files and an `editSpec.json` copy
- **Dependencies**: Remotion, React, and transition packages are CutFlow's own dependencies — no separate install needed

## FCPXML Output

Generates FCPXML 1.11 format XML. Maps clips to `<asset-clip>`, transitions to `<transition>`, text to `<title>`. Times expressed as rational numbers (e.g., `1001/30000s`). Each EditSpec outputs to `fcpxml/<name>.fcpxml`.
...
my-vlog-project/
  ...
  .cutflow/                     # Cache directory (in project directory)
    ...
    specs/
      <name>.json               # EditSpec JSON (written by `cutflow edit`)
    public/                     # Hard links to source video files + editSpec.json
      editSpec.json             # Copy of spec for Remotion Studio fallback
      video1.mp4                # Hard link to source video
  output/
    <name>.mp4                  # Generated by `cutflow render`
  fcpxml/
    <name>.fcpxml               # Generated by `cutflow edit` (per EditSpec)
```
