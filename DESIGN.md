...
`watchSegment`, `previewFrame`, and `previewFinalVideo` share a single 10-per-turn media budget (Gemini's per-request file-ref limit). `turn_start` fires before every LLM request rather than once per user message, so the counter resets for each new assistant message and the budget is a batch size: the agent can watch 10 segments, report what it found in them, and watch 10 more. `limitVideoFilesInContext` keeps only the 10 newest file refs across the whole conversation, replacing older ones with a placeholder — since a batch is evicted only after the agent has already seen it, the two limits line up and nothing is dropped unseen.

The per-batch report is a tool call rather than plain text because `gemini-3.8-flash` does not mix prose with tool calls: replaying a real batch boundary, it returned an empty text part in every sampled variant, whether the system prompt, the `watchSegment` description, or the tool result asked for a written summary. Offered a tool for the same content it called it every time, with or without prompt support. The argument is one free-text field: a per-segment schema was equally reliable, but prose reads as the agent talking and lets it rule a take out or compare it against another, and the description asking for video IDs and ranges is what keeps every watched segment accounted for.
...
Montai emits its own fade, slide, and pop keyframes because Subtitle's built-in animation duration scales with the title clip instead of Montai's fixed animation duration. These animations match Remotion and do not control the title's static layout.

DaVinci ignores the Subtitle Motion template's published layout and intrinsic animation adjustments; it still reads the embedded text style directly at 1×, so its existing centered/no-animation overlay limitation remains. Audio clips with volume and positioning import correctly into DaVinci, but fadeIn/fadeOut are ignored.
